...
$('#application, #rapplication').select2({
       minimumInputLength: 1,
       ajax: {
        url: site.base_url+"reports/suggestion",
        dataType: 'json',
        quietMillis: 15,
        data: function (term, page) {
            return {
...
$('#branch, #rbranch').select2({
       minimumInputLength: 1,
       ajax: {
        url: site.base_url+"reports/installment_suggestions",
        dataType: 'json',
        quietMillis: 15,
        data: function (term, page) {
            return {
...
function left_row(x) {
	return (x != null) ? '<div class="text-left">'+x+'</div>' : '';
}
...
function roundUpMoney(x, type) {
	
	if(type == 1){
		var devide = Math.round(x/100);
		var result = (devide * 100);
...
/*function CurrencyConvert(x = null){
	var  y = x.split("___");
	var amt = (y[1]/(site.settings.default_currency) * y[0]);
	return amt;
}*/
function pqFormat(x) {
    if (x != null) {
...
function row_status(x) {
    if(x == null) {
        return '';
    } else if(x == 'pending' || x == 'book' || x == 'free' || x == 'applicant') {
        return '<div class="text-center"><span class="label label-warning">'+lang[x]+'</span></div>';
    } else if( x == 'paid' || x == 'sent' || x == 'received' || x == 'activated' || x == 'approved_condition' || x == 'pending_po' || x == 'registered') {
        return '<div class="text-center"><span class="label label-success">'+lang[x]+'</span></div>';
    } else if(x == 'partial' || x == 'transferring' || x == 'ordered'  || x == 'busy'  || x == 'processing') {
        return '<div class="text-center"><span class="label label-info">'+lang[x]+'</span></div>';
    } else if(x == 'due' || x == 'returned' || x == 'rejected') {
...
function row_status_down(x) {
    if(x == null) {
        return '';
    } else if(x == 'pending' || x == 'book' || x == 'free' || x == 'quotation' || x == 'pending_po') {
        return '<div class="text-center"><span class="label label-warning">'+lang[x]+'</span></div>';
    } else if(x == 'paid' || x == 'sent' || x == 'received' || x == 'activated' || x == 'registered') {
        return '<div class="text-center"><span class="label label-success">'+lang[x]+'</span></div>';
    } else if(x == 'partial' || x == 'transferring' || x == 'ordered'  || x == 'busy'  || x == 'processing') {
        return '<div class="text-center"><span class="label label-info">'+lang[x]+'</span></div>';
    } else if(x == 'due' || x == 'returned' || x == 'rejected') {
...
$(document).ready(function() {
    ...
    $('body').on('click', '.invoice_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'Installment_payment/schedule_view/' + $(this).parent('.invoice_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'sales/view/' + $(this).parent('.invoice_link').attr('id');
    });
	...
	$('body').on('click', '.customer_details_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'Installment_payment/payment_schedule/' + $(this).parent('.customer_details_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'sales/view/' + $(this).parent('.invoice_link').attr('id');
    });
	/*$('body').on('click', '.payment_schedule_link td:not(:first-child :last-child)', function() {
		if($(this).parent('.payment_schedule_link').attr('status')) {
			window.location.href = site.base_url + 'Installment_payment/payment_schedule/' + $(this).parent('.payment_schedule_link').attr('id');
		}
    });*/
    ...
    $('body').on('click', '.quote_link td:not(:first-child :last-child)', function() {
		...
		if($(this).parent('.quote_link').attr('status')) {
			window.location.href = site.base_url + 'quotes/approvedApplicant/' + $(this).parent('.quote_link').attr('id');
		}
		// $('#myModal').modal('show');
		//window.location.href = site.base_url + 'quotes/view/' + $(this).parent('.quote_link').attr('id');
    });
	 $('body').on('click', '.quote_linkE td:not(:first-child :last-child)', function() {
        //$('#myModal').modal({remote: site.base_url + 'quotes/modal_view/' + $(this).parent('.quote_link').attr('id')});
		window.location.href = site.base_url + 'quotes/edit/' + $(this).parent('.quote_linkE').attr('id');
       // $('#myModal').modal('show');
        //window.location.href = site.base_url + 'quotes/view/' + $(this).parent('.quote_link').attr('id');
    });
	
	$('body').on('click', '.down_payment_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'down_payment/add_payment/' + $(this).parent('.down_payment_link').attr('id')});
        $('#myModal').modal('show');
    });
	
	$('body').on('click', '.down_saving_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'saving/add_saving/' + $(this).parent('.down_saving_link').attr('id')});
        $('#myModal').modal('show');
    });
	/*
	$('body').on('click', '.installment_link td:not(:first-child :last-child)', function() {
		window.location.href = site.base_url + 'Installment_payment/payment/' + $(this).parent('.installment_link').attr('id');
    });
	
	
	$('body').on('click', '.installment_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'Installment_payment/add_payment/' + $(this).parent('.installment_link').attr('id')});
        $('#myModal').modal('show');
    });
	*/
	...
	////////true
	/*$('body').on('click', '.installment_link td:not(:first-child :last-child)', function(){
		//var mfi=$(this).attr('mfi');
		var mfi=$(this).parent('.installment_link').attr('mfi');
		
		if(mfi==1){
			$('#myModal').modal({remote: site.base_url + 'Installment_payment/cash_payment_schedule_preview_by_id/' + $(this).parent('.installment_link').attr('id')});
			$('#myModal').modal('show');
		}else{
			$('#myModal').modal({remote: site.base_url + 'Installment_payment/payment_list/' + $(this).parent('.installment_link').attr('id')});
			$('#myModal').modal('show');
		}
    });*/
	...
	$('body').on('click', '.view_cash td:not(:first-child :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'Daily_Cash_Collection/view_cash/' + $(this).parent('.view_cash').attr('id')});
        $('#myModal').modal('show');
    });
	...
	$('body').on('click', '.contract_link td:not(:first-child :last-child)', function() {
		if($(this).parent('.contract_link').attr('mfi') != 1) {
			if($(this).parent('.contract_link').attr('status') == 'registered') {
				alert("This contract has been registered!");
			}else {
				$('#myModal').modal({remote: site.base_url + 'down_payment/register_form/' + $(this).parent('.contract_link').attr('id')});
				$('#myModal').modal('show');
			}
			//window.location.href = site.base_url + 'down_payment/contract_link/' + $(this).parent('.contract_link').attr('id');
		}
    });
	...
	/* Audit Popup */
	$('body').on('click', '.audit_link td:not(:last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'system_settings/audit_trial_details/' + $(this).parent('.audit_link').attr('id')});
        $('#myModal').modal('show');
    });
...
function formatDateTimeAMPM(date, second = false) {
	var d = new Date(date);
	var hh = d.getHours();
	var m = d.getMinutes();
	var s = d.getSeconds();
...
function getSelected2By1Child(url, child_obj, lang, pholder) {
	$('#modal-loading').show();
	$.ajax({
		type: "get",
		async: false,
		...
		error: function () {
			bootbox.alert('Error while AJAX request!');
			$('#modal-loading').hide();
		}
	});
	$('#modal-loading').hide();
...
function getSelected2By3Child(child_obj1, child_obj2, child_obj3, url, lang, pholder) {
	$('#modal-loading').show();
	$.ajax({
		type: "get",
		async: false,
		...
		success: function (scdata) {
			if (scdata != null) {
				$("#"+child_obj1).select2("destroy").empty().attr("placeholder", lang).select2({
					placeholder: pholder,
					data: scdata
				});
				$("#"+child_obj2).select2("destroy").empty().attr("placeholder", lang).select2({
					placeholder: pholder,
					data: scdata
				});
				$("#"+child_obj3).select2("destroy").empty().attr("placeholder", lang).select2({
					placeholder: pholder,
					data: scdata
				});
			}
...
function getInstallmentAmount(lease_amount, rate_type, interest, term) {
	var principle = 0;
	var interest_rate = 0;
	...
	if(rate_type == '1') {
		...
		installment_amount = ((lease_amount * interest)*((Math.pow((1+interest),term))/(Math.pow((1+interest),term)-1)));
		return installment_amount;
	} else if(rate_type == '3') {
		principle = lease_amount/term;
		interest_rate = lease_amount * interest;
...
function convertCurrency(to_rate, from_rate, amount){
	var convertAmount = (to_rate / from_rate) * amount;
	return convertAmount;
}
function getPaymentSchedule(sale_id = null, lease_amount = null, rate_type = null, interest = null, term_in_days = null, frequency = null, start_date = null)
{
	var term = (term_in_days/frequency).toFixed(0);
	start_date = moment(start_date).add(0,'days').format('DD/MM/YYYY');
	var payment_schedule = Array();
	if(rate_type == '1') {
		var principle = lease_amount/term;
		var lease_amt = lease_amount;
		var interest_rate = 0;
		var j=0;
		for(i=1; i<=term; i++) {
			if(i == '1') {
				...
				interest_rate = lease_amt * interest;
				var dateline = moment(start_date).add(j,'days').format('DD/MM/YYYY');
			}
			lease_amt -= principle;
			var payment_amt = principle + interest_rate;
			payment_schedule [j] = {
									period: i,
									sale_id: sale_id,
									type: rate_type,
		...
		var payment_amt = ((lease_amount * interest)*((Math.pow((1+interest), term))/(Math.pow((1+interest), term)-1)));
		var lease_amt = lease_amount;
		var interest_rate = 0;
		var j=0;
		for(i=1; i<=term; i++) {
			...
			var principle = payment_amt - interest_rate;
			lease_amt -= principle;
			payment_schedule [j] = {
									period: i,
									sale_id: sale_id,
...
function term_in_days(x) {
	return '<div style="text-align: right;">' + parseFloat(parseFloat(x).toFixed(0)) + ' Days</div>';
}

function terest_in_percent(x) {
	return '<div style="text-align: right;">' + parseFloat(x).toFixed(2) + '%</div>';
}
...
$(document).on('keypress keyup blur','.k_m_amount', function(event){	
