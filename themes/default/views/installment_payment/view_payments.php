...
<div id="none_" class="modal-dialog modal-lg" style="width:80%">
        ...
        <div class="modal-body print">
            <div class="table-responsive">
                <table id="CompTable" cellpadding="0" cellspacing="0" border="0"
                    ...
                    <thead>
                    <tr>
                        <th style="width:13%;"> <?= $this->lang->line("payment_date"); ?> </th>
						...
						<th style="width:8%;"> <?= $this->lang->line("penalty_"); ?> </th>
						<th style="width:8%;"> <?= $this->lang->line("other"); ?> </th>
						<th style="width:10%;"> <?= $this->lang->line("principles"); ?> </th>
                    ...
                    <?php if (!empty($payments)) {
                            ...
                            <tr class="row<?= $payment->id ?>">
                                ...
                                <td class="noprint">
                                    <div class="text-center">
                                        <?php
											$print_recipt = anchor('Installment_payment/payment_voucher/'.$payment->loan_id .'/'.$payment->sale_id .'/'.$payment->id, ' <i class="fa fa-print" style="font-size:20px; color:#1E90FF;" title="​'. lang("print_recipt") .'" ></i>','data-toggle="modal" data-target="#myModal2"');	
											
											if($payment->id == $last_payment->id){
												$edit_payment = anchor('Installment_payment/edit_payments/'.$payment->loan_id .'/'.$payment->sale_id .'/'.$payment->id, '<i class="fa fa-pencil-square" style="font-size:20px; color:#1E90FF;" title="​'. lang("edit_payment") .'"></i>','data-toggle="modal" data-target="#myModal2"');
											}else{
                        ...
                        echo "<tr><td colspan='11'>" . lang('no_data_available') . "</td></tr>";
                    } ?>
                    </tbody>
					<tfoot>
						<tr style="height:50px;">
							<td colspan="6" class="text-right">  </td>
							<td colspan="2" class="text-right"> <b> <?=$this->lang->line("total_payments"); ?> :  </b> </td>
							<td colspan="3" class="text-left"><?=  $this->erp->formatMoney($loan->paid_amount) ?></td>
							
						</tr>
						<tr>
							<td colspan="6" class="text-right">  </td>
							<td colspan="2" class="text-right"> <b> <?=$this->lang->line("owed_balances"); ?> : </b> </td>
							<td colspan="3" class="text-left"><?= $this->erp->formatMoney($loan->owed) ?></td>
