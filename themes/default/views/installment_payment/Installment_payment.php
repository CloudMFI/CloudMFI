...
class Installment_Payment extends MY_Controller
{
	...
	function add_payment($id = NULL, $loans_id = NULL) {
        ...
        if ($this->form_validation->run() == true) {
			...
			$interest_amount = str_replace(',', '', $this->input->post('interest_amount'));
			...
			if($payment_status == 'partial') {
				...
				$total_amt = $penalty_amount + $total_service_charge + $installment_amount + $other_amount - $paid;
				$balance = 0;
			}
			//$this->erp->print_arrays($total_amt);
			$payment = array(
								'date' => $pay_date,
								'sale_id' => $id,
								'loan_id' => $loans_id,
				...
				if($payment_status == 'partial') {
					if($total_amt > $_POST['installment'][$i] || ($total_amt > $_POST['owed'][$i] && $_POST['owed'][$i] > 0)) {
						...
						if($_POST['owed'][$i] > 0) {
							...
							$owed = ($_POST['installment'][$i]+$_POST['ovamounts'][$i]+$other_amount) - $total_amt;
						}
						$total_amt = 0;
					}
				}else {
        ...
        if ($this->form_validation->run() == true && $this->installment_payment_model->addPayment($payment) && $this->installment_payment_model->updateLoan($loan)) {
		  ...
		  redirect($_SERVER["HTTP_REFERER"]);
			...
			if($sale){
				...
				if($sale_item) {
					$this->data['loan_currency'] = $this->site->getCurrencyByCode($sale_item->default_currency);
				}
	...
	function payment_schedule($pdf = NULL, $excel = 1,$sale_id= NULL)
    {
		$this->erp->checkPermissions('Sales');
