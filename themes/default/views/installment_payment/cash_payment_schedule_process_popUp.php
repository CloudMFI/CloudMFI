					...
					<table style="font-size:11px;border-collapse:collapse;width:100%;" class="schedule">
						<tr class="p_l_r" style="background-color:#009900;color:white;">
							<?php 
								...
								if($DiffDate > $days){
									echo '<td class="t_c"><div  style="background-color: white; width: 10%; margin-left: 0;"><input type="checkbox" name="all_check" id="all_check"></div></td>' ;
								} else{
									echo '<td class="t_c"><div  style="background-color: white; width: 10%; margin-left: 0; pointer-events: none;"><input type="checkbox" name="all_check" id="all_check"></div></td>' ;
								}
							?>							
						...
						<?php
							...
							if(array($loan)) {
								...
								echo '<tr class="row-data" '.(($pt->paid_amount > 0)? 'style="background-color:#B4D8E8;"':'').'>
									...
									<td class="t_c" ><input type="checkbox" name="ch_check[]" class="ch_check" value="'.(($pt->paid_amount == 0)? $pt->id:'').'" '.(($pt->paid_amount > 0)? 'checked':'') .'></td>
									...
									foreach($services as $service){											
										if ($service->service_paid==2){	
											$haft = 0 ;
											if($service->method =="Percentage"){
												//$haft = ($service->charge_by == 1)? ($service->amount * $sale->total): (($service->charge_by == 2)? ($service->amount * $loan_balance ) : 0 ) ;	
											...
											echo'<td class="t_r" style="padding-left:5px; padding-right:5px;">'. (($pt->period >= 1 && $pt->period <= $countrow)? $haft_service_paid:'0.00') .'</td>';
										}
										
										else if ($service->service_paid==3){
											$alls = 0;
											if($service->method =="Percentage"){
												$alls = ($service->charge_by == 2)? ($service->amount * $loan_balance): ( $service->amount * $sale->total) ;											
											}else{
											...
											echo'<td class="t_r" style="padding-left:5px; padding-right:5px;">'. $all_service_paid .'</td>';
										}
											...
											echo'<td class="t_r" style="padding-left:5px;padding-right:5px; font-weight:normal">'. (($pt->period >= 1 && $pt->period <= $countrow)? $insurence_paid:'0.00') .'</td>';
										}											
									}									
									...
									$balances = (($pt->balance > 0)? $pt->balance : 0);
								echo '<td class="t_r" style="padding-left:5px;padding-right:5px;">'. $this->erp->roundUpMoney($balances, $sale_item->currency_code) .'</td>
									  <td class="t_r" style="padding-left:5px;padding-right:5px;">'. $this->erp->roundUpMoney($payment, $sale_item->currency_code) .'</td>
									 ';	
									...
									if($pt->id < $last_payment->loan_id){
										...
										if ($Owner || $Admin || $this->permission['payment-index']){
											$view_payment = anchor('Installment_payment/view_payments/'.$pt->id.'/'.$sale->id,'<i class="fa fa-money" style="font-size:18px; color:#1E90FF;"></i>','data-toggle="modal" data-target="#myModal2"');
										}
										/*if ($Owner || $Admin || $this->permission['payment-edit']) {
											$edit_payment = '<i class="fa fa-pencil-square" style="font-size:18px; pointer-events: none; color:#6495ED;" ></i>';
										}
										if($Owner || $Admin || $GP['installment-payment_voucher']){
											$print_recipt = anchor('Installment_payment/payment_voucher/'.$pt->id.'/'.$sale->id, '<i class="fa fa-print" style="font-size:18px; color:#1E90FF;" ></i>','data-toggle="modal" data-target="#myModal2"');
										}*/
									}
									...
									if ($pt->id == $last_payment->loan_id){
										if($pt->id == $last_payment->loan_id && $last_payment->owed > 0){
											if ($Owner || $Admin || $this->permission['payment-edit']){
											$re_payment = anchor('Installment_payment/re_payments/'.$pt->id.'/'.$sale->id, '<i class="fa fa-plus-square" style="font-size:18px; color:#1E90FF;"></i>','data-toggle="modal" data-target="#myModal2"');
											}
										}
										...
										/*if ($Owner || $Admin || $this->permission['payment-edit']){
											$edit_payment = anchor('Installment_payment/edit_payment/'.$pt->id.'/'.$sale->id, '<i class="fa fa-pencil-square" style="font-size:18px; color:#1E90FF;"></i>','data-toggle="modal" data-target="#myModal2"');
										}
										if($Owner || $Admin || $GP['installment-payment_voucher']){
											$print_recipt = anchor('Installment_payment/payment_voucher/'.$pt->id.'/'.$sale->id, ' <i class="fa fa-print" style="font-size:17px; color:#1E90FF;" ></i>','data-toggle="modal" data-target="#myModal2"');									
										}*/
									}
								...
								echo '<td class="t_r" id="hide_action2" style="padding-left:5px;padding-right:5px;"> '. $re_payment .' &ensp; '. $view_payment .'</td>
						...
						<tr class=" text-bold">
							...
							<td class="t_r" style="padding-left:5px;padding-right:5px;"><?= $this->erp->roundUpMoney($total_principle, $sale_item->currency_code); ?></td>
							<td class="t_r" style="padding-left:5px;padding-right:5px;"><?= $this->erp->roundUpMoney($total_interest, $sale_item->currency_code); ?></td>
							<?php
								foreach ($services as $service){
									if($service->service_paid==2) {
							...
							<td></td>
							<td class="t_r" style="padding-left:5px;padding-right:5px;"><?= $this->erp->roundUpMoney($total_pay, $sale_item->currency_code); ?></td>
							<td></td>
