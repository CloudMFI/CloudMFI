...
<script>
    ...
    $(document).ready(function () {
        var oTable = $('#QUData').dataTable({
            ...
            'sAjaxSource': '<?= site_url('quotes/getQuotesRejected'. ($warehouse_id ? '/' . $warehouse_id : '')).'/?v=1'.$v?>',
            ...
            "aoColumns": [{
                "bSortable": false,
                "mRender": checkbox
            }, null, null,null, null, {"bVisible": false}, {"mRender": row_status}, {"mRender": fld}, {"mRender": fld},null,null,{"mRender": currencyFormat},null, {"bSortable": false}],
			"fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var grand_total = 0;
                for (var i = 0; i < aaData.length; i++) {
					grand_total += parseFloat(aaData[aiDisplay[i]][11]);
                }
				var nCells = nRow.getElementsByTagName('th');
                nCells[10].innerHTML = currencyFormat(parseFloat(grand_total));
            }
            ...
            {column_number: 4, filter_default_label: "[<?=lang('asset');?>]", filter_type: "text", data: []},
            ...
            {column_number: 7, filter_default_label: "[<?=lang('submit_date');?> (yyyy/mm/dd)]", filter_type: "text", data: []},
            {column_number: 8, filter_default_label: "[<?=lang('appr/rej_date');?> (yyyy/mm/dd)]", filter_type: "text", data: []},
            <!--{column_number: 9, filter_default_label: "[<?=lang('acceptation_date');?>]", filter_type: "text", data: []},-->
            <!--{column_number: 10, filter_default_label: "[<?=lang('underwriter');?>]", filter_type: "text", data: []},-->
			{column_number: 9, filter_default_label: "[<?=lang('created_by');?>]", filter_type: "text", data: []},
			{column_number: 10, filter_default_label: "[<?=lang('branch');?>]", filter_type: "text", data: []},
...
<div class="box">
    <div class="box-header">
        <h2 class="blue">
		<?php if ($this->permission['reports-back_office']){ ?>
			<i class="fa-fw fa fa-heart-o"></i><?= lang('contracts_list'); ?>
		<?php }else{ ?>
			<i class="fa-fw fa fa-heart-o"></i><?= lang('rejected_list'); ?>
		<?php } ?>
        </h2>
                    ...
                    <ul class="dropdown-menu pull-right" class="tasks-menus" role="menu" aria-labelledby="dLabel">
						...
						<li>
                            <a href="<?= site_url('customers/send_sms'); ?>" data-toggle="modal" data-target="#myModal" id="SMS">
                                <i class="fa fa-send"></i> <?= lang("send_sms"); ?>
                            </a>
                        </li>
                    </ul>
                </li>
        ...
        <div class="row">
            <div class="col-lg-12">
				...
				<div id="form">
                    ...
                    <?php echo form_open("quotes/loan_rejected"); ?>
                    <div class="row">
						<div class="col-sm-4">
