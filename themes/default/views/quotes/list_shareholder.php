...
<script>
    ...
    $(document).ready(function () {
        var oTable = $('#QUData').dataTable({
            ...
            'sAjaxSource': '<?= site_url('quotes/getQuotes/'. $GP['view-draft']).'/?v=1'.$v?>',
            ...
            'fnRowCallback': function (nRow, aData, iDisplayIndex) {
				...
				$(nRow).attr("group_loan", aData[2]);
				$(nRow).attr("action", aData[12]);
				var action = $('td:eq(12)', nRow);
				
				if(aData[7] == 'draft' && aData[12]){
					nRow.className = "warning";
					action.find('.aa').remove();
				}
				
					if(aData[2] == null) {
						action.find('.app').remove();
					}
					if(aData[7] == "activated") {
            ...
            "aoColumns": [{
                "bSortable": false,
                "mRender": checkbox
            }, null,null,null,null, null, {"bVisible": false}, {"mRender": row_status}, {"mRender": fld}, {"mRender": fld},null,null,{"mRender": currencyFormat}, {"bSortable": false}],
			"fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var grand_total = 0;
                for (var i = 0; i < aaData.length; i++) {
					grand_total += parseFloat(aaData[aiDisplay[i]][12]);
                }
				var nCells = nRow.getElementsByTagName('th');
                nCells[11].innerHTML = currencyFormat(parseFloat(grand_total));
            }
			...
			{column_number: 12, filter_default_label: "[<?=lang('group_loan');?>]", filter_type: "text", data: []},
...
<div class="box">
    <div class="box-header">
		...
		<?php }else{ ?>
			<i class="fa-fw fa fa-heart-o"></i><?= lang('list_shareholder'); ?>
		<?php } ?>
        </h2>
