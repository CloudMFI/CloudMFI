...
<script>
    ...
    $(document).ready(function () {
        var oTable = $('#QUData').dataTable({
            ...
            'sAjaxSource': '<?= site_url('Reports/getSales_installment_report'. ($warehouse_id ? '/' . $warehouse_id : '')).'/?v=1'.$v?>',
            ...
            "aoColumns": [{
                "bSortable": false,
                "mRender": checkbox
            }, null, null, null,{"mRender": fld},{"mRender": currencyFormat}, null, 
            ...
            {column_number: 4, filter_default_label: "[<?=lang('payment_date');?>]", filter_type: "text", data: []},
            {column_number: 5, filter_default_label: "[<?=lang('amount_paid');?>]", filter_type: "text", data: []},
...
<?php if ($Owner) {
	    echo form_open('Reports/InstallmentReportsActions', 'id="action-form"');
	}
?>
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-heart"></i><?=lang('installment_report') . ' ' . ($warehouse_id ? $warehouse->name : lang('')) . '';?>
        </h2>
		<div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
            ...
            <ul class="btn-tasks">
                ...
                <?php if (!empty($warehouses)) {
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right" class="tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?=site_url('Reports')?>"><i class="fa fa-building-o"></i> <?=lang('all_warehouses')?></a></li>
                            <li class="divider"></li>
                            <?php
                            	foreach ($warehouses as $warehouse) {
                            	        echo '<li><a href="' . site_url('Reports/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
                            	    }
                                ?>
                        </ul>
                    </li>
    ...
    <div style="display: none;">
        <input type="hidden" name="form_action" value="" id="form_action" required="required"/>
        ...
        <div class="row">
            <div class="col-lg-12">
				...
				<div id="form">
                    ...
                    <?php echo form_open("Reports/installment_report"); ?>
                    <div class="row">
						<div class="col-sm-4">
								...
								<?php
									...
									if($customers) {
										foreach($customers as $cust) {
											...
											$all_cust[$cust->id] = $cust->family_name . " " . $cust->name . " | " .$cust->family_name_other . " " .$cust->name_other;
										}
									}
                        ...
                        <div class="col-sm-4">
							...
							<div class="form-group">
                                <label class="control-label" for="customer"><?= lang("by_c.o"); ?></label>
								<?php
                                $us[""] = "";
								if(is_array(isset($users) ?$users  : (''))){
                                foreach ($users as $user) {
                                ...
                                echo form_dropdown('user', isset($us) ?$us  : (''), (isset($_POST['user']) ? $_POST['user'] : ""), 'class="form-control" id="user" data-placeholder="' . $this->lang->line("select") . " " . $this->lang->line("user") . '"');
                                ?>
                                <!--<?php echo form_input('customer', (isset($_POST['customer']) ? $_POST['customer'] : ""), 'class="form-control" id="customer" data-placeholder="' . $this->lang->line("select") . " " . $this->lang->line("by_c.o") . '"'); ?>-->
                            </div>
                        ...
                        <div class="col-sm-4">                            
							<div class="form-group">
                                <label class="control-label" for="branch"><?= lang("by_branch"); ?></label>
                                <?php
                                $bn[""] = "";
                                ...
                                ?>
								<!--<?php echo form_input('customer', (isset($_POST['customer']) ? $_POST['customer'] : ""), 'class="form-control" id="branch" data-placeholder="' . $this->lang->line("select") . " " . $this->lang->line("by_branch") . '"'); ?>-->
                            </div>
                        </div>
	...
	/*$(document).ready(function(){
		$('#excel').on('click', function(e){
			...
			if ($('.checkbox:checked').length <= 0) {
				window.location.href = "<?=site_url('reports/getInstallmentReportAll/0/xls')?>";
				return false;
			}
		});
		$('#pdf').on('click', function(e){
			e.preventDefault();
			if ($('.checkbox:checked').length <= 0) {
				window.location.href = "<?=site_url('reports/getInstallmentReportAll/pdf/?v=1'.$v)?>";
				return false;
			}
		});
	});*/
