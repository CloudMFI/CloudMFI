...
class Accounts_model extends CI_Model
{
	...
	public function addDisbursement($data = array() , $services= array() ){
        if($data) {
			if($this->db->insert('payments',$data)){
				...
				//$this->erp->print_arrays($new_branchamount);
				$amount_payment = $this->erp->syncDisbursPayment($data['sale_id']);
				//$this->erp->print_arrays($amount_payment);
				$payment_status = '';
	...
	public function getAjaxSaleById($ids){
		$this->db->select('sales.total,sales.grand_total, sale_items.currency_code, sales.id, branch.amount as branch_amount, CONCAT(erp_companies.family_name," ",erp_companies.name) AS customer_name');
		$this->db->where('sales.id', $ids);
		$this->db->from('sales');
