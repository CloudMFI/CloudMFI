...
<?php //$this->erp->print_arrays($financial_detail); ?> 
<div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-2x">&times;</i>
            </button>
            <h4 class="modal-title" id="myModalLabel"><?php echo lang('add_financial'); ?></h4>
        </div>
        <?php $attrib = array('data-toggle' => 'validator', 'role' => 'form');
        echo form_open_multipart("system_settings/edit_financial/".(isset($financial_detail->id) ? $financial_detail->id : ''), $attrib); ?>
        <div class="modal-body">
			...
			<div class="form-group">
                <?= lang("amount", "amount"); ?>
                <input name="amount" type="text" id="amount" value="<?= (($financial_detail->method == 'Percentage')? $this->erp->formatNumber(($financial_detail->amount * 100)).'%' : $this->erp->formatDecimal($financial_detail->amount));?>" class="form-control" required="required"/>
            </div>
			<div class="form-group">
                <?= lang("status", "status"); ?>
                <?php
			...
			<div class="form-group all">
				<?= lang("follow_charge_by", "follow_charge_by"); ?>
				<div class="form-group all" style="border: solid 1px #ccc; height:40px; padding:10px;">					
					<div class="col-sm-4">
						<b>Loan Amount</b>	
						<input type="checkbox" name="loan_amount" id="loan_amount" class="check" <?php echo($financial_detail->charge_by == '1' ? 'checked="checked"':''); ?>/>
					</div>
					<div class="col-sm-4">
						<b>Remaing Balance</b>	
						<input type="checkbox" name="remaing_balance" id="remaing_balance" class="check" <?php echo($financial_detail->charge_by == '2' ? 'checked="checked"':''); ?>/>
					</div>
					<div class="col-sm-4">
						<b>Payment Amount</b>	
						<input type="checkbox" name="payment_amount" id="payment_amount" class="check" <?php echo($financial_detail->charge_by == '3' ? 'checked="checked"':''); ?>/>
					</div>
				</div>
			</div>
			<div class="form-group all">
			...
			<div class="form-group all">
				<div class="form-group">
					<?= lang("accrued_service","accrued_service"); ?>
					<?php
						$acc_section = array(""=>"");
						$get_accrued_service = "";
						foreach($accrued_service_income as $accrued_service){
							$get_accrued_service = $accrued_service->accountname;
						}
						foreach($chart_accounts as $section){
							$acc_section[$section->accountcode] = $section->accountcode.' | '.$section->accountname;
						}
						echo form_dropdown('accrued_service_income', $acc_section, $financial_detail->accrue_service ,'id="accrued_service_income " class="form-control" data-placeholder="' . $data->accrued_service_income . ' | ' . $this->lang->line($get_accrued_service) . '" style="width:100%;" required="required"');
					?>
				</div>
			</div>
			<div class="form-group all">
				<div class="form-group">
					...
					<?php
						...
						echo form_dropdown('state_tax',$tax, $financial_detail->tax_id, 'class="form-control" id="state_tax" data-placeholder="' . $financial_detail->tax_id . '" required="required"');
					?>
				</div>
			</div>
			<div class="form-group all">
