...
<script type="text/javascript">
	...
	$(window).load(function() {
		$('#interest_rate_cash_2').change(function() { 
			...
			$('.co-transfer').attr("readonly",false);
			$('.check').attr("disabled",true);
	});	
</script>
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-edit"></i><?= lang('co_transfer'); ?></h2>
    </div>
            ...
            <div class="col-lg-12">
				...
				<div class="col-md-12">
							...
							<div class="tab-content">
								...
								echo form_open_multipart("down_payment/co_transfer/" . $id, $attrib)
								?>
												...
												<div class="row">
														...
														<div class="col-md-6">
															<?php if (isset($this->permission['reports-underwriting']) ?$this->permission['reports-underwriting'] : ('') || $this->Admin || $this->Owner || $GP['quotes-edit']){ ?>
																	<div class="form-group">
																		<?= lang("co_transfer", "co_transfer"); ?>
																		<?php
																			$us[""] = "";
																			if(is_array(isset($users) ?$users  : (''))){
																			foreach ($users as $user) {
																				$us[$user->id .'#'. $user->branch_id] = $user->first_name . " " . $user->last_name;
																			}}
																			echo form_dropdown('cus_by_co', $us, $applicant->created_by, 'class="co-transfer" id="cus_by_co" placeholder="' . lang("Select_C.O_load") . '" ');																		
																		?>
																		<input type="hidden" name="branch_coid" id="branch_id">
																	</div>
																	<div class="form-group" style="display:none;">
																		<?= lang("status", "qstatus"); ?>
																		<?php
																		$status_q = array('' => '', 'applicant' => lang('applicant'),'draft' => lang('draft'),'approved' => lang('approved'),'activated' => lang('activated'));
													...
													<div class="col-lg-12">
														<div class="col-md-6">
															...
															<div class="form-group">
																<?= lang("family_name_en", "cus_family_name"); ?>
																<?php echo form_input('cus_family_name', $applicant->family_name, 'class="form-control tip" id="cus_family_name" '); ?>
															</div>
															<div class="form-group">
																<?= lang("first_name_en", "cus_first_name"); ?>
															   <?php echo form_input('cus_first_name', $applicant->name, 'class="form-control" id="cus_first_name"'); ?>
															</div>
															<div class="form-group">
...
<!-- get lat long  ---->
 <script>
	...
	var count_link=0;
		...
		$('#cus_by_co').live('change', function() {
			var branch_name = $(this).val();
			var branch_id = branch_name.split('#');
			var branch_co = branch_id[1];
			$('#branch_id').val(branch_co);
		});
