...
function getBillReciept($pdf = NULL, $xls = NULL)
{
	...
	function add_deposit($sl_id = NULL)
	{
		...
		if ($this->form_validation->run('account/add') == true) {
			...
			$paid_amount1 = $paid_amount - $owed_paid;			
			///////////////
			$interest_paid = 0;
			if($paid_amount1 > $interest_amount){
				$interest_paid = $interest_amount;
			}else{
				$interest_paid = $paid_amount1;
			}
			//$this->erp->print_arrays($interest_amount,___, $interest_paid);
			$owed_interest = $interest_amount - $interest_paid;
			$paid_amount2 = $paid_amount1 - $interest_paid;			
			///////////////
			$total_services_paid = 0;
			if($paid_amount2 > $total_services_amount){
				$total_services_paid = $total_services_amount;
			}else{
				$total_services_paid = $paid_amount2;
			}
			...
			$paid_amount3 = $paid_amount2 - $total_services_paid;
			///////////////
			$penalty_paid = 0;
			if($paid_amount3 > $penalty_amount){
				$penalty_paid = $penalty_amount;
			}else{
				$penalty_paid = $paid_amount3;
			}
			
			$owed_penalty = $penalty_amount - $penalty_paid;
			$paid_amount4 = $paid_amount3 - $penalty_paid;
			///////////////
			$other_paids = 0;
			if($paid_amount4 > $other_paids_amount){
				$other_paids = $other_paids_amount;
			}else{
				$other_paids = $paid_amount4;
			}
			
			$owed_other_paid = $other_paids_amount - $other_paids;
			$paid_amount5 = $paid_amount4 - $other_paids;			
			///////////////
			$priciple_paid = 0;
			if($paid_amount5 > $principle_amount){
				$priciple_paid = $principle_amount;
			}else{
				$priciple_paid = $paid_amount5;
			}
			
			$owed_principle = $principle_amount - $priciple_paid;
			//$this->erp->print_arrays($owed_principle);			
			$data = array	(			
								'date' 				=> $this->erp->fld(trim($this->input->post('date'))),
								'reference_no' 		=> $this->input->post('reference'),
								'sale_id'			=> $sale_id,
		...
		if ($rows = $this->accounts_model->getAjaxSaleById($sale_id)) {
			...
			$currency = $rows->currency_code;
				foreach($services as $service) {
					if($service->service_paid == 1) {
						if($service->type == 'Percentage'){
							$amount = $service->amount * $rows->total;
			...
			$total_amount = $this->erp->roundUpMoney($total_amount, $curency);			
            echo json_encode(array('total'=>$total_amount, 'service'=>$total_service_charges , 'one_services'=> $one_service, 'g_total'=>$remian_total, 'branch_amount'=>$branch_amount, 'def_rate'=>$def_rate->rate, 'sale_rate'=>$sale_currency->rate, 'customer'=>$rows->customer_name));
        } else {
            echo json_encode(false);
        }
