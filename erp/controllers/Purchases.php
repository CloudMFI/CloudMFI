...
class Purchases extends MY_Controller
{
    ...
    {
        $this->erp->checkPermissions('list_ac_payable', true,'account');
		if ($this->input->get('product')) {
            $product = $this->input->get('product');
        } else {
            $product = NULL;
...
//generate pdf and force to download
    ...
    {
        $this->erp->checkPermissions('expenses',null,'purchases');
    ...
    public function getExpenses($id = null)
    {
        ...
        $this->datatables
            ->select($this->db->dbprefix('expenses') . ".id as id, date, reference, gl_trans.narrative ,expenses.amount ,note, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ', " . $this->db->dbprefix('users') . ".last_name) as user,erp_companies.name, erp_expenses.attachment", false)
            ->from('expenses')
    ...
    {
        $this->erp->checkPermissions('add',false,'purchases');
        $this->load->helper('security');
        ...
        if ($this->form_validation->run() == true) {
			...
			$exp_currency = $this->input->post('currency');
			$currency = $this->input->post('currency');
			$currency_code = explode('#', $currency); 
			$exp_currency = $currency_code[0];
			
			$settings = $this->site->get_setting();
			$df_currency = $settings->default_currency;
			$amount = str_replace(',', '', $this->input->post('amount'));
			
            $data_payment = array(
                            'date'          => $date,
							'reference_no'	=> $this->site->getReference('pay'),
							'amount'        => $this->erp->convertCurrency($df_currency, $exp_currency, $amount ),
							'paid_type'		=> 'Expense',
                            'paid_by'       => 'cash',
                            'created_by'    => $this->session->userdata('user_id'),
							'biller_id'		=> $this->input->post('branch'),                            
							'type'			=> 'sent',
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addExpense($data, $data_payment)) {
			...
			$this->data['df_currency'] = $this->purchases_model->getCurrencyByCode($setting->default_currency);
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
	...
	public function ajaxBranchBalance($branch_id, $bank_code){
		$this->load->model('purchases_model');
		if ($rows = $this->purchases_model->getAjaxBranchBalance($branch_id, $bank_code)) {
