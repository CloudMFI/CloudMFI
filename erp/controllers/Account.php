...
class Account extends MY_Controller
{
	...
	function settings()
	{
		$this->erp->checkPermissions('settings',false,'account');
		if($this->input->post('update_settings')){
			...
			if($this->input->post('default_money_transfer') == null){
				$dmoney_transfer = $this->input->post('dmoney_transfer');
			}else{
				$dmoney_transfer = $this->input->post('default_money_transfer');
			}
			...
			if($this->input->post('default_penalty_income') == null){
				$penalty_income = $this->input->post('penalty_income');
			}else{
				$penalty_income = $this->input->post('default_penalty_income');
			}
			if($this->input->post('default_interest_income') == null){
				$interest_income = $this->input->post('interest_income');
			}else{
				$interest_income = $this->input->post('default_interest_income');
			}
			if($this->input->post('default_other_income') == null){
				$other_income = $this->input->post('other_income');
			}else{
				$other_income = $this->input->post('default_other_income');
			}
			if($this->input->post('default_capital') == null){
				$default_capital = $this->input->post('capital');
			}else{
				$default_capital = $this->input->post('default_capital');
			}
			if($this->input->post('accrued_interest') == null){
				$accrued_interest = $this->input->post('c_interest');
			}else{
				$accrued_interest = $this->input->post('accrued_interest');
			}
			if($this->input->post('default_transfer_money') == null){
				$transfer_money = $this->input->post('transfer_money');
			}else{
		...
		$this->data['money_transfer'] = $this->accounts_model->getmoney_transfer();
		$this->data['cheque'] = $this->accounts_model->getcheque();
		$this->data['loan'] = $this->accounts_model->get_loan();
		$this->data['other_income'] = $this->accounts_model->get_other_income();
		$this->data['default_capital'] = $this->accounts_model->get_default_capital();
		$this->data['accrued_interest'] = $this->accounts_model->get_accrued_interest();
		$this->data['transfer_money'] = $this->accounts_model->get_default_transfer_money();
	...
	{
		$this->erp->checkPermissions('list_receivable', false, 'account');
	...
	function list_ar_aging() {
		$this->erp->checkPermissions('list_ar_aging',false,'account');
		$this->data['user_id'] =isset( $user_id) ? $user_id  : ('');
...
function getChartAccount()
{
	...
	$this->load->library('datatables');
	$this->datatables
	->select("(erp_gl_charts.accountcode) as id,erp_gl_charts.accountcode, erp_gl_charts.accountname,erp_gl_charts.accountname_kh, erp_gl_charts.parent_acc, erp_gl_sections.sectionname")
	->from("erp_gl_charts")
...
{
	$this->erp->checkPermissions('bill_receipt', true, 'account');
	$this->load->model('reports_model');
	$this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
	$this->data['users'] = $this->reports_model->getStaff();
	$this->data['billers'] = $this->site->getAllCompanies('biller');
...
function getBillReciept($pdf = NULL, $xls = NULL)
{
	...
	{
		$this->erp->checkPermissions('list_ap_aging', true,'account');
	...
	{
		$this->erp->checkPermissions('bill_payable', true, 'account');
		$this->load->model('reports_model');
		$this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
		$this->data['users'] = $this->reports_model->getStaff();
		$this->data['billers'] = $this->site->getAllCompanies('biller');
	...
	function getJournalList()
	{
		...
		->select("gt.tran_id, gt.tran_no AS g_tran_no, companies.company, gt.tran_type, gt.tran_date, 
			...
			gt.narrative,gt.created_name, gt.description, 
	...
	public function add_journal(){
		...
		$this->data['billers'] = $this->site->getAllCompanies('biller');
		$this->data['reference_no'] = $this->site->getReference('jr');
	...
	function save_journal(){
		...
		$reference_no = $this->input->post('reference')?$this->input->post('reference'):$this->site->getReference('jr');
	...
	public function updateJournal(){
		...
		$gltans = $this->accounts_model->getDelJournalByTranNo($tran_no_old, $tran_id);
		$not_account = array();
		foreach($gltans as $gl){
			$not_account[] = $gl->tran_id;
		}
		$data = array();
		for($i=0;$i<count($account_code);$i++) {
			if($debit[$i]>0) {
	...
	function getReferenceByBranch($field){
		$reference_no = $this->site->getReference($field);
	...
	function add()
	{
		$this->erp->checkPermissions('add_ac_head',false, 'account');
    ...
    public function getDeposits(){
    	...
    	<ul class="dropdown-menu pull-right" role="menu">
    		...
    		$this->load->library('datatables');
    		$this->datatables
    		->select("deposits.id as dep_id, companies.id AS id , date,companies.name, companies.deposit_amount, paid_by, CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) as created_by", false)
    		->from("deposits")
	...
	{
		$this->erp->checkPermissions('add',false,'disbursement');
			...
			if($services){
				...
				for($i = 0; $i < $n; $i++){
					$arr_services[] = array(
						...
						'amount' 	=> $this->erp->convertCurrency($default_currency->default_currency, $saleItem->currency_code, $services[$i])
					);
				}
			}
		...
		if ($this->form_validation->run() == true && $sid = $this->accounts_model->addDisbursement($data, $arr_services)) {
			$this->session->set_flashdata('message', $this->lang->line("accound_added"));
			$ref = isset($_SERVER["HTTP_REFERER"]) ? explode('?', $_SERVER["HTTP_REFERER"]) : NULL;
			redirect($ref[0] . '?account=' . $sid);
		} else {
			...
			//$this->erp->print_arrays($user->branch_id);
			$this->data['reference_sp'] = $this->site->getReference('pp');
			...
			$this->data['contracts'] = $this->accounts_model->getContractDiburse($user->branch_id);
			$this->data['sectionacc'] = $this->accounts_model->getAccountSections();
			$this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
			$this->data['modal_js'] = $this->site->modal_js();
			$this->load->view($this->theme . 'accounts/add_disbursement', $this->data);
		}
	}
	
	function add_deposit($sl_id = NULL)
	{
		$this->erp->checkPermissions('add',false,'deposit');
		$this->form_validation->set_rules('contract_id', $this->lang->line("contract_id"), 'required');
		$this->form_validation->set_rules('date', $this->lang->line("date"), 'required');
		$this->form_validation->set_rules('paid', $this->lang->line("paid"), 'required');
		...
		if ($this->form_validation->run('account/add') == true) {
			...
			$principle = str_replace(',', '', $this->input->post('principle'));
			$interest = str_replace(',', '', $this->input->post('interest'));
			$penalty = str_replace(',', '', $this->input->post('penalty'));
			$total_services = str_replace(',', '', $this->input->post('total_services'));
			$other_paid = str_replace(',', '', $this->input->post('other_paid'));
			$owed_paid = str_replace(',', '', $this->input->post('owed_balance'));
			$total = str_replace(',', '', $this->input->post('amount'));
			
			$saleItem = $this->accounts_model->getSaleItemBysaleID($sale_id);	
			$sale = $this->accounts_model->getSaleById($sale_id);
			$default_currency = $this->site->get_setting();
			$df_currency = $default_currency->default_currency;
			$loan_currency = $saleItem->currency_code;
			
			$paid_amount = $this->erp->convertCurrency($df_currency, $loan_currency, $paid);
			$balance_amount = $this->erp->convertCurrency($df_currency, $loan_currency, $balance);			
			$principle_amount = $this->erp->convertCurrency($df_currency, $loan_currency, $principle);
			$interest_amount = $this->erp->convertCurrency($df_currency, $loan_currency, $interest);
			$penalty_amount = $this->erp->convertCurrency($df_currency, $loan_currency, $penalty);
			$total_services_amount = $this->erp->convertCurrency($df_currency, $loan_currency, $total_services);
			$other_paids_amount = $this->erp->convertCurrency($df_currency, $loan_currency, $other_paid);
			$owed_paids_amount = $this->erp->convertCurrency($df_currency, $loan_currency, $owed_paid);
			$total_payments = $this->erp->convertCurrency($df_currency, $loan_currency, $total);
			//$this->erp->print_arrays($paid_amount, $total_payments);
			$services = str_replace(',', '', $this->input->post('service'));
			$service_id = $this->input->post('service_id');
			
			$payment_status = "";
			if($paid_amount < $total_payments){
			...
			if($paid_amount > $owed_paids_amount){
				$owed_paid = $owed_paids_amount;
			}else{
				$owed_paid = $paid_amount;
			}
			
			$bl_from_owedPaid = $paid_amount - $owed_paid;			
			///////////////
			$interest_paid = 0;
			...
			//$this->erp->print_arrays($interest_amount,___, $interest_paid);
			$owed_interest = $interest_amount - $interest_paid;
			$bl_from_interestPaid = $bl_from_owedPaid - $interest_paid;			
			///////////////
			$total_services_paid = 0;
			if($bl_from_interestPaid > $total_services_amount){
				$total_services_paid = $total_services_amount;
			}else{
				$total_services_paid = $bl_from_interestPaid;
			}
			//$this->erp->print_arrays($total_services_amount,___, $total_services_paid);
			//$this->erp->print_arrays($total_services_amount);
			$owed_services = $total_services_amount - $total_services_paid;
			...
			///////////////
			$other_paids = 0;
			if($bl_from_penaltyPaid > $other_paids_amount){
				$other_paids = $other_paids_amount;
			}else{
				$other_paids = $bl_from_penaltyPaid;
			}
			
			$owed_other_paid = $other_paids_amount - $other_paids;
			$bl_from_otherPaid = $bl_from_penaltyPaid - $other_paids;			
			///////////////
			$priciple_paid = 0;
			if($bl_from_otherPaid > $principle_amount){
				$priciple_paid = $principle_amount;
			}else{
			...
			//$this->erp->print_arrays($arr_services);
			$old_payment = $this->accounts_model->getLastOldPayment($sale_id);
			...
			$old_services = $this->accounts_model->getServicepayment($old_payment->id);
			...
			//$this->erp->print_arrays($arr_oldservices);
			/*$arr_services = array();
			if($services) {
				$n = sizeof($services);
				for($i = 0; $i < $n; $i++) {
		...
		if ($this->form_validation->run() == true && $sid = $this->accounts_model->addDeposit($data, $arr_services, $old_payments, $arr_oldservices)) {
			$this->session->set_flashdata('message', $this->lang->line("accound_added"));
			$ref = isset($_SERVER["HTTP_REFERER"]) ? explode('?', $_SERVER["HTTP_REFERER"]) : NULL;
			redirect($ref[0] . '?account=' . $sid);
		} else {
			$this->data['sale_id']	= $sl_id;
			$this->data['sales'] = $this->accounts_model->getSaleById($sale_id);
			$this->data['reference_sp'] = $this->site->getReference('sp');
			$this->data['banks'] = $this->accounts_model->getBankAccount();
			$this->data['currencies'] = $this->accounts_model->getCurrncy();
			$this->data['contracts'] = $this->accounts_model->getContract();
			$this->data['sectionacc'] = $this->accounts_model->getAccountSections();
			$this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
			$this->data['modal_js'] = $this->site->modal_js();
			$this->load->view($this->theme . 'accounts/add_deposit', $this->data);
		}
	}
	public function ajaxGetLoanBysaleID($sale_id = NULL){
		if ($rows = $this->accounts_model->getloanBySaleID($sale_id)) {
			...
			$default_currency = $this->site->getCurrencyByCode($setting->default_currency);
			$sale = $this->accounts_model->getSaleByID($sale_id);
			$service_payment = $this->accounts_model->getServicesPaymentBySaleID($sale_id);
			$owes_bl = $this->accounts_model->getLoanOwedBySaleId($sale_id);
			$sumOweds = $owes_bl->owed;
			//$coutperiod = $this->accounts_model->getLoanperiodBySaleId($sale_id);
			if($sale){
				$sale_item = $this->accounts_model->getSaleItemBySaleID($sale->id);
				$sale = $sale;
				$sale_item = $sale_item;
				if($sale_item) {
					$loan_currency = $this->site->getCurrencyByCode($sale_item->currency_code);
					$sale_rate = $loan_currency->rate;
				}
				$services = $this->accounts_model->getServicesBySaleID($sale->id);
				$count = $this->installment_payment_model->getLoanBySaleId($sale->id);
				
			}
			$loans = $this->installment_payment_model->getLoanByID($rows->id);	
			$getLoan = $this->installment_payment_model->getLoanCurrency($rows->id);
			$currency_type = $getLoan->type;
			...
			foreach($loans as $loan) {
				...
				if($loan->paid_amount > 0) {
					...
					if($final_dateline < $current_date) {
						...
						$ovamounts = $ovdays * $this->erp->convertCurrency($getLoan->code, $setting->default_currency, $penalty_rate);
						$overdue_days += $ovdays;
					}else{
						$ovdays = 0;
						$ovamounts = 0;
				...
				foreach($services as $service) {
					if($service->service_paid != 1) {
						...
						$total_service_charge += (($service->service_paid == 2 && $loan->period <= $countrow)? ($service->amount):(($service->service_paid == 3)? ($service->amount):0) ?:(($service->service_paid == 4 && $loan->period <= $countrow )? (($service->amount) / $countrow):0));
					}
				}
				*/
				$loan_balance = $rows->balance - $rows->principle;
				$service->amount = 0;
				$total_service_charge = 0;
				...
				foreach($services as $service) {					
					...
					if($service->service_paid == 4 && $loan->period <= $countrow){
						if($service->type == "Percentage"){
							...
							$service_amount = $amount + ($amount * $service->tax_rate);  //(($loan->period <= $countrow)? ($service_amount) : 0 );
							
						}else{
							$amount = $this->erp->convertCurrency($getLoan->code, $setting->default_currency, $service->amount) / $countrow ;
							$service_amount = $amount + ($amount * $service->tax_rate);
						}
					}
			...
			$total_service_charges = $this->erp->roundUpMoney($total_service_charge,$getLoan->code);
			$principles = $this->erp->roundUpMoney($rows->principle ,$getLoan->code);
			$interests = $this->erp->roundUpMoney($rows->interest ,$getLoan->code);
			...
			$date_now = date('Y-m-d');	// compare
			$ol_date = date($rows->dateline);	// compare
			
			if(strtotime($ol_date) < strtotime($date_now)){
				$paymentDate = $new_date;
			}else {
	...
	public function ajaxGetSaleBysaleID($sale_id = NULL){
		...
		if ($rows = $this->accounts_model->getAjaxSaleById($sale_id)) {
			...
			$one_service = $this->accounts_model->getOneServicesBySaleID($sale_id);
			$default_currency = $this->site->getCurrencyByCode($setting->default_currency);
			$currency = $rows->currency_code;
				foreach($services as $service) {
					if($service->service_paid == 1) {
						$loan_amt = $this->erp->convertCurrency($currency, $setting->default_currency, $rows->total);
						if($service->type == 'Percentage'){
							$amount = $service->amount * $loan_amt;
							$service->amount = $amount + ( $amount * $service->tax_rate);
						}
						else{
							$amount = $this->erp->convertCurrency($currency, $setting->default_currency, $service->amount);
							$service->amount = $amount + ( $amount * $service->tax_rate);
						}
			...
			$paid  = $rows->grand_total;
			$paids = $this->erp->convertCurrency($currency, $setting->default_currency, $paid);			
			$total_amount = $totals - $paids;
