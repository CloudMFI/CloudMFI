...
class Products extends MY_Controller
{
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            ...
            $this->datatables
                //->select($this->db->dbprefix('products') . ".id as productid, ". $this->db->dbprefix('products') . ".code as code, " . $this->db->dbprefix('products') . ".name as name,".$this->db->dbprefix('subcategories').".name as subcname, " . $this->db->dbprefix('categories') . ".name as cname, COALESCE(". $this->db->dbprefix('products') . ".quantity, 0) as quantity ", FALSE)
				->select($this->db->dbprefix('products') . ".id as productid, ". $this->db->dbprefix('products') . ".code as code, " . $this->db->dbprefix('products') . ".name as name,".$this->db->dbprefix('subcategories').".name as subcname, " . $this->db->dbprefix('categories') . ".name as cname")
                ->from('products')
                ->join('categories', 'products.category_id=categories.id', 'left')
				->join('subcategories', 'products.subcategory_id=subcategories.id', 'left');
				//->join('type', 'type.subcategory_id=subcategories.id', 'left');
        ...
        $this->datatables->add_column("Actions", $action, "productid, code, name,subcname,cname");
    ...
    function suggestions()
    {
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $pr[] = array('id' => $row->id, 'label' => $row->name . " (" . $row->code . ")(". $row->unit .")", 'uom' => $uom, 'code' => $row->code, 'name' => $row->name, 'price' => $row->price, 'qty' => 1, 'unit' => $row->unit);
				$uom = '';
            }
            echo json_encode($pr);
        } else {
...
/*	function import_csv(){
        ...
        if ($this->form_validation->run() == true) {
            if (isset($_FILES["userfile"]))
            {
				...
				$keys = array('product_code','product_name', 'type', 'category_id', 'subcategroy_id','product_detail', 'activation');
				$final = array();
				...
				{	
					...
					if($product_id) {
						$products[] = array(
								'code' => $csv_pr['product_code'],
								'name' => $csv_pr['product_name'],
								'type' => $csv_pr['type'],
								'category_id' => $csv_pr['category_id'],
								'subcategory_id' => $csv_pr['subcategroy_id'],
								'product_details' => $csv_pr['product_details'],
								'inactived' => 1,
									);		
				...
				if ($this->form_validation->run() == true && $this->products_model->add_products()) {
...
*/	
    ...
    function import_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $keys = array('product_code','product_name', 'type', 'category_id', 'subcategroy_id','product_details', 'inactived');
        ...
        if ($this->form_validation->run() == true && $this->products_model->ImportProduct($data)) {
            $this->session->set_flashdata('message', lang("products_added"));
            redirect('products');
        } else {
    ...
    function modal_view($id = NULL)
    {
        ...
        $q = $this->db->get()->result();
		$this->data['model'] = $this->site->getModelByID($pr_details->type);
	...
	function getTypes($subcategory_id = NULL)
    {
        if ($rows = $this->products_model->getTypeID($subcategory_id)) {
            $data = json_encode($rows);
        } else {
            $data = false;
        }
	...
	public function getListConvert()
    {
        ...
        $this->datatables
            ...
            ->group_by('convert.id');
			if ($this->permission['products-index'] = ''){
				if (!$this->Owner && !$this->Admin) {
	...
	function add_modal($id = NULL)
   {
        $this->erp->checkPermissions('add',TRUE,'products');
        $this->load->helper('security');
        $warehouses = $this->site->getAllWarehouses();
        if ($this->input->post('type') == 'standard') {
            $this->form_validation->set_rules('cost', lang("product_cost"), 'required');
        ...
        if ($this->form_validation->run() == true) {
			...
			$data = array(
                ...
                'engline' => $this->input->post('engline'),
                'frame' => $this->input->post('frame'),
                'chassis' => $this->input->post('chassis'),
                'product_details' => $this->input->post('product_details'),
				'power' => $this->input->post('power'),
                'product_year' => $this->input->post('product_year'),'distance_mile'    => $this->input->post('distance'),
				'inactived' => $inactived
			);
			//$this->erp->print_arrays($data);
            $this->load->library('upload');
        ...
        if ($this->form_validation->run() == true && $pid= $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos)) {
            $this->session->set_flashdata('message', lang("product_added"));
			
			 $ref = isset($_SERVER["HTTP_REFERER"]) ? explode('?', $_SERVER["HTTP_REFERER"]) : NULL;
            redirect($ref[0] . '?product_add=' . $this->input->post('code'));
            ...
            $this->data['modal_js'] = $this->site->modal_js(); 
			$meta = array('page_title' => lang(''), 'bc' => $bc);
			$this->load->view($this->theme . 'products/add_modal', $this->data);
        }
    }
	...
	public function edit_product($id=NULL){
			...
			$this->data['currencies'] = $this->products_model->getAllCurrencies();
            $this->data['categories'] = $this->site->getAllCategories();
            $this->data['type'] = $this->products_model->getpType();
			
		
			if( $this->data['product'] = $this->products_model->getEditProductByID($id)){
				
			$bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('products'), 'page' => lang('products')), array('link' => '#', 'page' => lang('edit_product')));
            $meta = array('page_title' => lang(''), 'bc' => $bc);
			$this->page_construct('products/edit_product', $meta, $this->data);
	...
	function updateProduct($id = NULL)
    {
		...
		if($this->products_model->update_product($id,$getProduct)){
