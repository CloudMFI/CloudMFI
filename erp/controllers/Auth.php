...
class Auth extends MY_Controller
{
    ...
    function getUsers(){
        ...
        $this->datatables
            ->select($this->db->dbprefix('users').".id as id,".$this->db->dbprefix('users').".id as userid,CONCAT(erp_users.first_name,' ',erp_users.last_name) AS user_name,users.username as username, ".$this->db->dbprefix('users').".email as email, ".$this->db->dbprefix('companies').".name as br_name, " . $this->db->dbprefix('groups') . ".name, active")
            ->from("users")
            ->join('groups', 'users.group_id=groups.id', 'left')
			->join('companies','users.branch_id = companies.id','left')
            ->group_by('users.id')
    ...
    {
        if (!$this->ion_auth->logged_in() || !$this->ion_auth->in_group('owner') && !$this->ion_auth->in_group('admin') && $id != $this->session->userdata('user_id')) {
            $this->session->set_flashdata('warning', lang("access_denied"));
            redirect($_SERVER["HTTP_REFERER"]);
        }
        if (!$id || empty($id)) {
        ...
        $this->data['branch_id'] = $this->site->getBranchByUserID($id);
    ...
    function create_user($id)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $additional_data = array(
                ...
                'commission' => $this->input->post('commission_percentage'),
