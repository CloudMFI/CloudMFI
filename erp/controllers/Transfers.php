...
class Transfers extends MY_Controller
{
    ...
    function add()
    {
        ...
        if ($this->form_validation->run() == true && $this->transfers_model->addTransfer($data, $products)) {
			...
			$this->data['branch'] = $this->site->getBranchDName();
	...
	/*function insert(){
		$data = array(
			'date' 			=> $this->erp->fld(trim($this->input->post('date'))),
			'from_branch_id'=>$this->input->post('from_warehouse'),
			'to_branch_id'	=>$this->input->post('to_warehouse'),
		...
		$this->load->model('transfers_model');
		$i=$this->transfers_model->insert($data);
		if($i){
			$this->session->set_flashdata('Money transfer successfull.');
			redirect('branch');
		}
	}*/		
