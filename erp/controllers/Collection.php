...
class Collection extends MY_Controller
{
    function __construct()
    {
        ...
        $this->lang->load('quotations_lang', $this->Settings->language);
    ...
    function index($warehouse_id = NULL, $b_id=NULL)
    {
		...
		$this->data['by_co'] = $this->site->getByCO_Name();
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('bad_loans')));
        $meta = array('page_title' => lang('bad_loans'), 'bc' => $bc);
        $this->page_construct('collection/index', $meta, $this->data);
    }
    ...
    {
        $this->erp->checkPermissions('index',false,'collection');
        ...
        $detail_link = anchor('collection/contract_detail/$1', '<i class="fa fa-file-text-o"></i> ' . lang('contract_details'));
        ...
        $payment_schedule = anchor('Installment_Payment/payment_schedule/0/1', '<i class="fa fa-file-pdf-o"></i> ' . lang('payment_schedule'));
        ...
        $delete_link = "<a href='#' class='po' title='<b>" . lang("delete_sales") . "</b>' data-content=\"<p>"
        ...
        $this->load->library('datatables');
            ...
            $this->datatables
                ->select("".$this->db->dbprefix('loans').".sale_id as id, ".
						...
						(IF(DATE_ADD(".$this->db->dbprefix('loans').".dateline, INTERVAL ".$penalty_days." DAY) < CURDATE(), (DATEDIFF(CURDATE(), ".$this->db->dbprefix('loans').".dateline)) * ".$penalty_amount.", '') *(".$this->db->dbprefix('currencies').".rate / ".$setting->rate .")) as penalty_amount,
	...
	function contract_detail($combine_id = NULL)
	{
		
       $this->erp->checkPermissions();
		
		$ids = explode('___', $combine_id);
		$id = $ids[0];
		...
		$this->page_construct('collection/view_contract', $meta, $this->data);
	}
	...
	function add_payment($group_id=NULL) {

		$this->erp->checkPermissions('payments', true);
        $this->load->helper('security');

		$ids = explode('___', $group_id);
		$loan_id = $ids[0];
		$id = $ids[1];
		...
		$this->form_validation->set_rules('due_date', lang("installment_date"), 'required');
        ...
        if ($this->form_validation->run() == true) {
			...
			$total_service_charge = $this->input->post('total_service_charge');
			...
			$total_amt = $penalty_amount + $total_service_charge + $installment_amount + $other_amount;
			$payment = array(
								'date' => $pay_date,
								'sale_id' => $sale_id,
								'loan_id' => $loan_id,
        ...
        if ($this->form_validation->run() == true && $this->Installment_Payment_model->addPayment($payment) && $this->Installment_Payment_model->updateLoan($loan)) {
          //  $this->session->set_flashdata('message', lang("payment_added"));
          //  redirect($_SERVER["HTTP_REFERER"]);
		  $this->data['PaymentByRef']=$this->Installment_Payment_model->getPaymentByRef($reference_no);
		  $this->data['service']=$this->Installment_Payment_model->getServiceByID($sale_id);

		 // $this->erp->print_arrays($this->data['PaymentByRef']);
			$this->data['modal_js'] = $this->site->modal_js();
		  $this->data['product']=$this->Installment_Payment_model->getItemByIDs($sale_id);
		  $this->data['exchange_rate_kh_c'] = $this->pos_model->getExchange_rate('KHM');

		  $this->load->view($this->theme.'installment_payment/report',$this->data);
        } else {
			...
			$this->data['loan'] = $this->Installment_Payment_model->getLoanByID($loan_id);
	...
	function payment_schedule($pdf = NULL, $excel = 1)
    {
        ...
        if ($pdf || $excel) {
          $this->db->where('loans.sale_id',4);
            $this->db
              ->select('
              products.product_year as product_year,
              products.power as power,
              ...
              loans.dateline as dateline,
              loans.id as id
              ')
              ->from('loans')
              ->join('sales','sales.id=loans.sale_id')
            ...
            if (!empty($data)) {
				...
				$this->excel->getActiveSheet()->SetCellValue('C7', lang($data[0]->customer));

				$this->excel->getActiveSheet()->SetCellValue('A8', lang('អាសយដ្ឋាន'));
				$this->excel->getActiveSheet()->SetCellValue('C8', lang($data[0]->address));
				...
				$this->excel->getActiveSheet()->SetCellValue('C10', lang(' '.$data[0]->phone));
				$this->excel->getActiveSheet()->SetCellValue('F10', lang('LID Number:'));
				$this->excel->getActiveSheet()->SetCellValue('G10', lang($data[0]->lid));
				...
				$this->excel->getActiveSheet()->SetCellValue('D15', lang($data[0]->product_name));
				...
				$this->excel->getActiveSheet()->SetCellValue('D16', lang($data[0]->product_year));
				...
				$this->excel->getActiveSheet()->SetCellValue('D17', lang($data[0]->power));
				...
				$this->excel->getActiveSheet()->SetCellValue('D20', lang('561.00%'));
        ...
        foreach ($data as $dt) {
          ...
          $this->excel->getActiveSheet()->SetCellValue('D'.$rownum.'', lang('ប្រាក់ផ្ទែរកម្មសិទ្ធិ'));
          ...
          $this->excel->getActiveSheet()->SetCellValue('A'.$rownum.'', lang($dt->id));
  				$this->excel->getActiveSheet()->SetCellValue('B'.$rownum.'', lang($dt->dateline));
  				...
  				$this->excel->getActiveSheet()->SetCellValue('I'.$rownum.'', $this->erp->moneyFormat($dt->balance));
	...
	function phone_collection($warehouse_id = NULL)
    {
		$this->erp->checkPermissions();
		$this->load->model('reports_model');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('collection')));
        $meta = array('page_title' => lang('collection'), 'bc' => $bc);
        $this->page_construct('collection/phone_collection', $meta, $this->data);
    }
	function getPhoneCollection($warehouse_id = NULL)
    {
        $this->erp->checkPermissions();
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('sales').".id,".
						...
						$this->db->dbprefix('sales').".phone_status, ".
						//$this->db->dbprefix('users').".username as phone_collector_name,".
						$this->db->dbprefix('loans').".dateline,
						NOW(), ".
						$penalty_days .",
						IF(DATE_ADD(".$this->db->dbprefix('loans').".dateline, INTERVAL ".$penalty_days." DAY) < CURDATE(), DATEDIFF(CURDATE(), ".$this->db->dbprefix('loans').".dateline), '') as due_days, ".
						...
						(".$this->db->dbprefix('loans').".payment + (SELECT SUM(amount) FROM ".$this->db->dbprefix('sale_services')." WHERE sale_id = ".$this->db->dbprefix('sales').".id) + IF(DATE_ADD(".$this->db->dbprefix('loans').".dateline, INTERVAL ".$penalty_days." DAY) < CURDATE(), (DATEDIFF(".$this->db->dbprefix('loans').".dateline, CURDATE())) * ".$penalty_amount.", '')) as total_amount, ")
                ->from('sales')
				->join('loans','sales.id=loans.sale_id','INNER')
				->join('companies', 'sales.customer_id = companies.id', 'INNER')
				->join('users','sales.phone_collector_name=users.id','LEFT')
                ->where('warehouse_id', $warehouse_id)
				->where('sales.sale_status', 'activated')
				->where('loans.dateline <=', date('Y-m-d'))
				->where('loans.paid_amount', 0)
            ...
            $this->datatables
                ->select("CONCAT(".$this->db->dbprefix('loans').".id, '___', ".$this->db->dbprefix('loans').".sale_id) as id, ".
						$this->db->dbprefix('sales').".reference_no,
						CONCAT(".$this->db->dbprefix('companies').".family_name, ' ', ".$this->db->dbprefix('companies').".name) as customer_name,
						CONCAT(".$this->db->dbprefix('companies').".family_name_other, ' ', ".$this->db->dbprefix('companies').".name_other) as customer_name_other,
						IF(".$this->db->dbprefix('companies').".phone2<>'', CONCAT(".$this->db->dbprefix('companies').".phone1, ' / ', ".$this->db->dbprefix('companies').".phone2), ".$this->db->dbprefix('companies').".phone1) as phone, ".
						$this->db->dbprefix('sales').".biller, IF (phone_status = NULL,'',phone_status),".
		...
		function add_phone_collection($group_id=NULL) {
        ...
        if ($this->form_validation->run() == true && $this->Installment_Payment_model->addPayment($payment) && $this->Installment_Payment_model->updateLoan($loan)) {
		 ...
		 // $this->erp->print_arrays($this->data['PaymentByRef']);
		  ...
		  $this->load->view($this->theme.'installment_payment/report',$this->data);
            ...
            $sale = $this->down_payment_model->getSaleByID($id);
            $this->data['sale'] = $sale;
			$this->data['services'] = $this->down_payment_model->getSaleServices(isset($sale->id) ?$sale->id  : (''));
			$this->data['customers'] = $this->down_payment_model->getAllCustomerCompanies();
			$this->data['loan'] = $this->Installment_Payment_model->getLoanByID($loan_id);
			$this->data['setting'] = $this->settings_model->getSettings();
			$this->data['down_percentages'] = $this->site->getAllDownPercentage();
            ...
            $this->load->view($this->theme.'collection/add_phone_collection', $this->data);
	
        }
	}
			
	public function  insert_phone_collection($id){
		
		
		$status = array(
								'phone_status' => $this->input->post('status'),
								'phone_collector_name'=> $this->session->userdata('user_id'),
								);
							$i=$this->site->updatePhoneCollectionStatus($status,$id);
							if($i){
								redirect('collection/phone_collection');
							}	
	}
