...
class Billers extends MY_Controller
{
    ...
    function getBillers()
    {
        ...
        $this->datatables
            ->select($this->db->dbprefix('companies').".id, company, ".$this->db->dbprefix('companies').".name, vat_no, phone, email, ".$this->db->dbprefix('provinces').".name_other as province, ".$this->db->dbprefix('cities').".name_other as city, bank_name, account_number, account_name")
            ->join('provinces', 'companies.state = provinces.id', 'left')
			->join('cities', 'companies.city = cities.id', 'left')
			->from("companies")
